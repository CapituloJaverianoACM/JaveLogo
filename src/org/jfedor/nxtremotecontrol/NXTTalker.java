...
public class NXTTalker {
    ...
    public void moveFront(int power,boolean reverse, boolean reverseLR) {
    	this.move(power, 1, 1, reverse, reverseLR);
    }
    
    public void moveLeft(int power,boolean reverse, boolean reverseLR) {
    	//Log.i("NXT", "onTouch event: " + Integer.toString(event.getAction()));
        //if ((action == MotionEvent.ACTION_DOWN) || (action == MotionEvent.ACTION_MOVE)) {
    	this.move(power, -0.7, 0.7, reverse, reverseLR);
    	
    }
    public void moveBack(int power,boolean reverse, boolean reverseLR) {
    	//Log.i("NXT", "onTouch event: " + Integer.toString(event.getAction()));
        //if ((action == MotionEvent.ACTION_DOWN) || (action == MotionEvent.ACTION_MOVE)) {
    	this.move(power, -1, -1, reverse, reverseLR);
    }
    
    public void moveRight(int power,boolean reverse, boolean reverseLR) {
    	//Log.i("NXT", "onTouch event: " + Integer.toString(event.getAction()));
        //if ((action == MotionEvent.ACTION_DOWN) || (action == MotionEvent.ACTION_MOVE)) {
    	this.move(power, 0.7, -0.7, reverse, reverseLR);
    }
    
    public void move(int power, double l, double r, boolean reverse, boolean reverseLR) {
    	this.stopMovement();
        if (reverse) {
            power *= -1;
        }
        ...
        r*=power;
        if (reverseLR) {
            motors((byte)l,(byte) r, false, false);
        } else {
            motors((byte)r,(byte) l, false, false);
        }
    }
    public void stopMovement() {
    	this.motors3((byte)0,(byte) 0, (byte)0, false, false);
    }
